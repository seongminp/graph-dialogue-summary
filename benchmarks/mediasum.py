...
if __name__ == "__main__":
    ...
    for name, content in raw_data.items():
        ...
        transcript = ""
        dialogue = []
        for speaker, utterance in zip(content["speaker"], content["utt"]):
            dialogue.append((speaker, utterance))

        data.append({"dialogue": dialogue, "summaries": [content["summary"]]})
    ...
    for d in data_iterator:
        ...
        # candidates = [(sum, scr) for sum, scr in zip(summaries, scores)]
        # candidates.sort(key=lambda x: x[1], reverse=True)
        # summary = ' '.join(c[0] for c in candidates[3:])
        ...
        if verbose:
            print(f"[Mediasum] R1: {avg_r1:.4f}, R2: {avg_r2:.4f}, Rl: {avg_rl:.4f}")
    ...
    print(
        f"[Mediasum Final] R1: {final_r1:.4f}, R2: {final_r2:.4f}, Rl: {final_rl:.4f} (top_k: {args.top_k})"
    )
